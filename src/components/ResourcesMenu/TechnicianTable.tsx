...
import { SanitizeString } from "./ResourcesUtils";
import "./Resource.css";
import { GET_ALL_TECHNICIANS } from "../../api/queryList";
import { ComboBoxSelector } from "./ResourcesUtils";
    ...
    return (
        <>
            <form onSubmit={handleSubmit} className="form-style">
                ...
                <div>
                    <label htmlFor="count" className="label-style">
                        Count
                    </label>
                    <input type="number" name="count" id="count" value={formData.count} onChange={handleChange} className="text-input" />
                    {errors.count && <span className="error-message">{errors.count}</span>}
                </div>
                <div>
                    <label htmlFor="maxWorkHours" className="label-style">
                        maxWorkHours
                    </label>
                    <input type="number" name="maxWorkHours" id="maxWorkHours" value={formData.maxWorkHours} onChange={handleChange} className="text-input" />
                    {errors.maxWorkHours && <span className="error-message">{errors.maxWorkHours}</span>}
                </div>
                <button type="submit" onClick={() => setSubmitAction("submit")} className="submit-button" value="submit">
                    Submit
                </button>
                <button type="submit" onClick={() => setSubmitAction("delete")} className="delete-button" value="delete">
                    Delete
                </button>
                <div>
