...
export function TableLogic<TData, TValue>({ columns, data, renderExpandedComponent }: DataTableProps<TData, TValue>) {
    ...
    return (
        <div>
                    ...
                    <div>
                        ...
                        <Popover>
                            <PopoverTrigger asChild>
                                <button className="bg-blue-500 text-white font-bold py-2 px-4 rounded capitalize hover:bg-blue-700">Change Filters</button>
                            </PopoverTrigger>
                                        ...
                                        <div>
                                            <label>
                                                <input
                                                    {...{
                                                        type: "checkbox",
                                                        checked: table.getIsAllColumnsVisible(), //determine if all columns are currently visible.
                                                        onChange: table.getToggleAllColumnsVisibilityHandler(), //get the handler function to toggle the visibility of all columns.
                                                    }}
                                                />{" "}
                                                Toggle All
                                            </label>
                                                ...
                                                <div key={column.id}>
                                                    <label>
                                                        <input
                                                            {...{
                                                                type: "checkbox",
                                                                checked: column.getIsVisible(), //determine if the current column is visible.
                                                                onChange: column.getToggleVisibilityHandler(), //get the handler function to toggle the visibility of the current column.
                                                            }}
                                                        />{" "}
                                                        {column.id
                                                            .replace(/([A-Z])/g, " $1")
                                    ...
                                    return (
                                        <TableHead key={header.id} style={{ fontWeight: "bold" }}>
                                            {header.isPlaceholder
                                                ? null
                                                : flexRender(
                                                      //renders the header by using the columnDef.header function and the header context
                                    ...
                                    rowElements.push(
                                        <TableRow key={`extra ${row.id}`}>
                                            <TableCell colSpan={columns.length}>
                                                {renderExpandedComponent ? renderExpandedComponent(row.original) : null}
                                            </TableCell>
                                        </TableRow>
                                    );
                                }
            ...
            <div className="flex items-center justify-start space-x-2 py-4">
                ...
                <div>
                    <StyledButton onClick={handlePageSizeChange} disabled={!table.getCanNextPage()}>
                        Set Page Size
                    </StyledButton>
                    <input
